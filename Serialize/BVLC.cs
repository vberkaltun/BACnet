...
namespace System.IO.BACnet.Serialize
{
	...
	public class BVLC
	{
		...
		{
			Encode(buffer, BacnetBvlcFunctions.BVLC_DISTRIBUTE_BROADCAST_TO_NETWORK, msgLength);
			_myBbmdTransport.Send(buffer, msgLength, bbmd);

		}

		// Encode is called by internal services if the BBMD is also an active device
		public int Encode(byte[] buffer, BacnetBvlcFunctions function, int msgLength)
		{
		...
		public int Decode(byte[] buffer, out BacnetBvlcFunctions function, out int msgLength, IPEndPoint sender)
		{
			// offset always 0, we are the first after udp
